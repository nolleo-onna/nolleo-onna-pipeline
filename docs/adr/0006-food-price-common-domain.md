...
- 상태: Accepted
- 일자: 2026-05-31
- 적용 범위: `fd_food_places`, `fd_food_place_sources`, `fd_food_place_menus`,
...
음식/가격 데이터는 착한가격 전용 모델이 아니라 공통 `fd_food_*` 모델로 저장한다.

1. **장소 마스터 분리**
   - `fd_food_places`는 외부 음식/가격 장소의 내부 마스터다.
   - 업소명, 업종, 주소, 전화번호, 영업시간 원문, 대표 메뉴, 좌표, 활성 상태를 보관한다.
   - 코스 식사 후보로 쓸 수 있는지는 `is_course_food_candidate`로 구분한다.

2. **원천 payload 보존**
   - `fd_food_place_sources`에 원천 종류, 외부 ID, 지역, raw JSON, fetched_at을 저장한다.
...
3. **현재 메뉴/가격과 관측 이력 분리**
   - `fd_food_place_menus`는 서비스 조회용 현재 메뉴/가격이다.
   - `fd_food_price_observations`는 API/file/admin/user/crawler 관측 이력과 검수 상태를 보관한다.
   - 승인된 관측만 현재 메뉴 가격 반영 대상으로 본다.

4. **TourAPI spots 매칭은 별도 테이블로 관리**
   - `fd_food_place_spot_matches`가 `fd_food_places`와 `spots`의 매칭을 담당한다.
...
5. **추천 조회 캐시는 SoT가 아니다**
   - `sp_spot_price_summary`는 추천 쿼리 최적화용 캐시다.
   - source of truth는 `fd_food_place_menus`와 `fd_food_price_observations`다.
...
- `sp_spot_price_summary` 재계산 기준과 주기를 별도 운영해야 한다.
