...
| `0007` | 음식/가격 공통 도메인 `fd_food_*`, `sp_spot_price_summary` |
| `0008` | 부산 `ldong_codes` |
...
| 착한가격업소 API/파일 | 원천별 `external_id` | `fd_food_place_sources.raw_json` | `fd_food_places`, `fd_food_place_menus`, `fd_food_price_observations` |
| RedTable/부산맛집/모범음식점 등 | 원천별 `external_id` | `fd_food_place_sources.raw_json` | `fd_food_places`, `fd_food_place_menus`, `fd_food_price_observations` |
| 운영자 수기 가격 | 내부 place/menu | `fd_food_price_observations.raw_payload` | 승인 후 `fd_food_place_menus` 반영 |
...
- `fd_food_place_sources`: `(source, external_id)`
- `fd_food_place_menus`: `(food_place_id, menu_name, source)`
...
- `fd_food_price_observations`: 가격 관측/검수 이력
...
가격 정보는 `spots`에 직접 넣지 않고 `fd_food_places` 도메인으로 관리합니다.
...
### fd_food_place_spot_matches

`fd_food_places`와 TourAPI `spots`의 매칭 테이블입니다.

- `pending`, `matched`, `rejected`는 `spot_content_id IS NOT NULL`
- `separate`는 `spot_content_id IS NULL`
- 이 규칙은 CHECK constraint로 강제합니다.

### sp_spot_price_summary

추천 조회 최적화용 캐시입니다.

- source of truth가 아닙니다.
- `fd_food_place_menus`, `fd_food_price_observations`를 기준으로 재계산할 수 있어야 합니다.
